...
| 11 | Fixed worker consumer name shared across replicas → double-delivery of pending messages under horizontal scale | High (≥2 replicas) | **Fixed 2026-07-15**: `CascadeWorker` defaults to a per-process consumer identity (`worker-<host>-<pid>`); crashed processes' pending messages recover via the periodic `reclaim_stale` sweep. |
| 14 | Operational cold-chain alerts with bare temp_c had diagnosis/severity/time-to-unsafe/outage-duration FABRICATED into real decision evidence (guard required only temp_c; cascade defaulted the rest to story constants) | Critical (operational domain) | **Fixed 2026-07-15**: `_require_operational_context` now requires every field the cascade consumes for COLD_CHAIN_ALERT (mirroring what the edge/resilience layer genuinely computes via `diagnose()`/`predict_time_to_unsafe()`); live events fail closed with the missing fields named instead of being embellished. Regression-tested. |
| 15 | Hardcoded-value sweep (2026-07-15, full src/): markdown candidate (20%/24h) buried in cascade + agentic prompts → moved to the product-policy layer (per-family, zero behavior change); golden/cold-chain sim story physics → named `_*_STORY_*` constants, sim-domain-only by the #14 guard; scenario-drill wait (fixed 15s) → derived from the request budget (`_scenario_drill_wait_seconds`, env-overridable, capped below the 504 deadline); worker poll/reclaim-sweep frequencies and connector poll cadence → env-tunable (`SHELFWISE_WORKER_POLL_SECONDS`, `SHELFWISE_WORKER_RECLAIM_INTERVAL_SECONDS`, `CONNECTOR_POLL_INTERVAL_SECONDS`). Deliberately KEPT hardcoded, with rationale: protective ceilings (context/body/stream/page caps — guardrails, not triggers), named module constants with single ownership (`PRICE_EXCEPTION_TOLERANCE`), deterministic-evidence confidence annotations, and reproducibility seeds. Verified: no automatic production path fabricates pipeline events — events enter only via /ingest, connectors, edge gateway, or the explicit write-gated scenario-drill surface; the cold-chain feed writes to its own display buffer only. | — | **Closed** |
| 13 | Reclaim idle threshold hardcoded at 30s — INSIDE the 120s cascade budget, so the sweep could steal a live message from a healthy worker mid-cascade and double-run it (the same arbitrary-30-seconds failure mode as the retired submission gate) | Critical (worker mode) | **Fixed 2026-07-15**: `stale_consumer_idle_ms()` derives the threshold from the real work budget (`SHELFWISE_REQUEST_TIMEOUT_SECONDS` + persistence margin, default 240s); `SHELFWISE_WORKER_RECLAIM_IDLE_SECONDS` can raise but is clamped up to the derived floor so a sub-budget value is not expressible even through config. Sweep *frequency* (30s) is unchanged — checking often is harmless; presuming death early is not. Regression-tested. |
