...
387/387 tests pass. `python -m pytest -q` and `python -m ruff check .` (only cosmetic E501s
remain). Capability manifest is in sync (`python scripts/compare_capability_manifests.py --write`
...
AMD Developer Cloud MI300X droplet: `165.245.130.225` (SSH as root works from this machine,
key `~/.ssh/id_ed25519`). Inside it, Docker container `rocm` runs vLLM 0.23.0 (ROCm) serving
`google/gemma-4-E4B-it` on port 8000 with the Gemma tool parser. `.env` in repo root points
at it and is verified working (`/inference/readiness` → `ready_for_amd_demo: true`).
...
- **A v2 15-min run was IN FLIGHT when this session ended** (`full_capacity_v2.log`,
  artifacts in `data/harness_runs/full_capacity_15min_v2/`). Check it: expect
  `FULL_CAPACITY_V2_PASSED` at the end and chat_model_answers ≈ chat_calls (proving the
  /chat fix at scale). If it was killed mid-run, just rerun the same command (see git log
  message of `561c50b` or the inline python in this file's history).

## 7 real bugs found by live testing this session (all fixed + regression-tested)
...
6. `/chat` sent unbounded decision/learning history → prompt growth → timeout → silent
   offline fallback after ~cycle 6 of a long run. Now bounded (pending always kept in full).
...
1. Check the v2 full-capacity run result (see above).
