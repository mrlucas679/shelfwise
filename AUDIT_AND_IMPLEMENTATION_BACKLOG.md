...
| SEC-08 | **State-changing demo/worldgen endpoints are unauthenticated, unthrottled, and mutate via GET.** `/demo/golden|procurement|sales|cold-chain` expose GET handlers calling `decision_store.upsert`; `GET /demo/worldgen/{id}` runs a full ingest pipeline (up to 500 events) anonymously. GET mutation is cacheable/CSRF-reachable. | Med | S–M | HIGH | `app.py:704-764,767-846` |
...
| DEBT-07 | **Duplicated GET/POST `/demo/*` handlers** — 8 handlers where 4 (`api_route`) would do. | DRY | S | HIGH | `app.py:704-760` |
...
4. **Gate/secure demo & worldgen write endpoints** (SEC-08). Attach `write_path_guard`+rate-limit,
   make mutation POST-only, cap worldgen work/request. *Done when:* anonymous `GET /demo/*` no longer
...
26. **Decompose the god modules** (DEBT-01, DEBT-02, DEBT-05, DEBT-07). Extract per-domain routers +
    ...
    singletons. *Done when:* `/demo/*` responses are byte-identical (captured characterization tests)
