...
function InfoRow({ label, value, tone }: { label: string; value?: ReactNode; tone?: Tone }) {
  return (
    <div className="set-row set-row-static">
      <span className="set-label">{label}</span>
      <span className={`set-value${tone ? ` tone-${tone}` : ''}`}>{value ?? '-'}</span>
    </div>
  )
}
            ...
            {page === 'settings' ? (
              <section className="rail-section">
                ...
                <div className="sidebar-section tight">
                  <div className="sidebar-kicker">Inference</div>
                  <InfoRow label="Provider" value={inferenceProviderLabel(data?.inference)} tone={inferenceTone(data?.inference)} />
                  <InfoRow label="Compute" value={data?.inference?.compute_resource ?? '-'} />
                  <InfoRow label="Accelerator" value={data?.inference?.accelerator || 'Not active'} />
                  <InfoRow label="Endpoint" value={data?.inference?.base_url_configured ? 'Configured' : 'Not set'} tone={data?.inference?.base_url_configured ? 'ok' : 'mute'} />
                  <InfoRow label="Timeout" value={data?.inference?.timeout_seconds ? `${data.inference.timeout_seconds}s` : '-'} />
                </div>
                <dl className="kv">
                  <div>
                    <dt>Store</dt>
